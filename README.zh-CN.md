...
这个仓库的目标是沉淀方法论，不公开具体业务内容。我们先后尝试并长期维护了两个 LLM-Wiki，在持续使用中积累了大量工程问题与修复经验。这个项目把可复用部分抽象出来，形成可迁移的规则与流程：
...
项目缘起很直接：两个 LLM-Wiki 的长期运行暴露出同类型问题，我们希望把这些问题的解决路径沉淀成一套可复用方法。这里公开的是方法、规则和验证框架，不包含原始 Wiki 的私有内容：
